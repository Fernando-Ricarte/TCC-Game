...
{
	estado = stateError.seguindo;
}

// --------- animações -------------------//

switch(estado){
	case stateError.seguindo:
			...
			if(chao_previsao){
				if(!chega_perto_demais){
					...
					sprite_index = spr_errorIdle;
				}
			}else{
				estado = stateError.parado;
				sprite_index = spr_errorIdle;
			}
		
		break;
	case stateError.parado:
